...
metallic_meta <- data.frame(country=c("Austria", "Belgium", "Denmark", "Finland", 
                                     "France", "Germany","Ireland", "Italy", "Luxembourg", 
                                     "Netherlands", "Portugal", "Spain", "Sweden"),
                           stream="metallic",
                           model=c('SVM', 'XARIMA', 'BSTS', 'SVM', 'SVM', 'XARIMA',
                                   'SVM', 'Cubist', 'SVM', 'SVM','Cubist', 'SVM', 'SVM'),
                           n_covariates=c(2, 1, 1, 2, 4, 2, 2, 3, 2, 1, 2, 4, 2)
)

#2023 forcast error & absolute forecast error----
metallic_meta$forecast_error_2023 <- c(metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Austria-austria_median_pis[austria_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Belgium-belgium_median_pis[belgium_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Denmark-denmark_median_pis[denmark_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Finland-finland_median_pis[finland_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$France-france_median_pis[france_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Germany-germany_median_pis[germany_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Ireland-ireland_median_pis[ireland_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Italy-italy_median_pis[italy_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Luxembourg-luxembourg_median_pis[luxembourg_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Netherlands-netherlands_median_pis[netherlands_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Portugal-portugal_median_pis[portugal_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Spain-spain_median_pis[spain_median_pis$year=='2023',]$median_forecast,
                                      metallic_packaging_waste[metallic_packaging_waste$TIME=='2023',]$Sweden-sweden_median_pis[sweden_median_pis$year=='2023',]$median_forecast
)

metallic_meta$abs_forecast_error_2023 <- abs(metallic_meta$forecast_error_2023)

#PI width----
metallic_meta$pi_width_2023 <- c(austria_median_pis[austria_median_pis$year=='2023',]$upper_pi-austria_median_pis[austria_median_pis$year=='2023',]$lower_pi,
...
metallic_meta$train_sd <- c(sd(austria[austria$Year<=2017,]$mpw),
                           sd(belgium[belgium$Year<=2017,]$mpw),
                           sd(denmark[denmark$Year<=2017,]$mpw),
                           sd(finland[finland$Year<=2017,]$mpw),
                           sd(france[france$Year<=2017,]$mpw),
                           sd(germany[germany$Year<=2017,]$mpw),
                           sd(ireland[ireland$Year<=2017,]$mpw),
                           sd(italy[italy$Year<=2017,]$mpw),
                           sd(luxembourg[luxembourg$Year<=2017,]$mpw),
                           sd(netherlands[netherlands$Year<=2017,]$mpw),
                           sd(portugal[portugal$Year<=2017,]$mpw),
                           sd(spain[spain$Year<=2017,]$mpw),
                           sd(sweden[sweden$Year<=2017,]$mpw)
)

metallic_meta$test_sd <- c(sd(austria[austria$Year>=2018,]$mpw),
                          sd(belgium[belgium$Year>=2018,]$mpw),
                          sd(denmark[denmark$Year>=2018,]$mpw),
                          sd(finland[finland$Year>=2018,]$mpw),
                          sd(france[france$Year>=2018,]$mpw),
                          sd(germany[germany$Year>=2018,]$mpw),
                          sd(ireland[ireland$Year>=2018,]$mpw),
                          sd(italy[italy$Year>=2018,]$mpw),
                          sd(luxembourg[luxembourg$Year>=2018,]$mpw),
                          sd(netherlands[netherlands$Year>=2018,]$mpw),
                          sd(portugal[portugal$Year>=2018,]$mpw),
                          sd(spain[spain$Year>=2018,]$mpw),
                          sd(sweden[sweden$Year>=2018,]$mpw)
)

metallic_meta$sd_ratio <- metallic_meta$test_sd/metallic_meta$train_sd

#trend of training set, trend of whole series, their difference, binary change of trend direction----
trend_slope <- function(df, year_col = "Year", y_col = "mpw"){
  coef(lm(df[[y_col]] ~ df[[year_col]]))[2]
}

metallic_meta$slope_full <- c(trend_slope(austria), trend_slope(belgium),
...
metallic_meta$slope_diff <- metallic_meta$slope_full - metallic_meta$slope_train


metallic_meta$trend_reversal <- as.integer(sign(metallic_meta$slope_full)!=sign(metallic_meta$slope_train))

#maximum and mean absolute change (Rise/fall) over the whole series----
max_abs_change <- function(df, y_col = "mpw"){
  max(abs(diff(df[[y_col]])))
}

mean_abs_change <- function(df, y_col = "mpw"){
  mean(abs(diff(df[[y_col]])))
}

metallic_meta$max_abs_change <- c(max_abs_change(austria), max_abs_change(belgium),
...
metallic_meta$mean_abs_change <- c(mean_abs_change(austria), mean_abs_change(belgium),
...
write.csv(metallic_meta, file='metallic_meta_dataset.csv')
